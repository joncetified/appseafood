            ...
            <tbody>
                @forelse($testimonials as $testimonial)
                    <tr class="border-t border-slate-100">
                        ...
                        @if($isSuperAdmin)
                            <td class="px-6 py-4">{{ $testimonial->creator?->name ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $testimonial->updater?->name ?? '-' }}</td>
                        @endif
                ...
                @empty
                    <tr><td colspan="{{ $isSuperAdmin ? 6 : 4 }}" class="px-6 py-8 text-center text-slate-400">Belum ada testimoni.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
