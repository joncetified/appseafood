            ...
            <tbody>
                @forelse($orders as $order)
                    <tr class="border-t border-slate-100">
                        ...
                        @if($isSuperAdmin)
                            <td class="px-6 py-4">{{ $order->creator?->name ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $order->updater?->name ?? '-' }}</td>
                        @endif
                        <td class="px-6 py-4 text-right"><a href="{{ route('admin.orders.show', $order) }}" class="font-medium text-cyan-700">Detail</a></td>
                    </tr>
                @empty
                    <tr><td colspan="{{ $isSuperAdmin ? 8 : 6 }}" class="px-6 py-8 text-center text-slate-400">Belum ada pesanan.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
